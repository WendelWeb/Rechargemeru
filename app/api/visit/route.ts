 ...
 * Body: `{ path, referrer?, locale?, utm? }` as JSON — read from raw text, so a
 * `text/plain` body parses too.
 *
...
import { db, pgErrorCode, schema } from '@/db';
import {
  DEVICE_COOKIE,
...
/**
 * What a failed insert may say in the logs. drizzle's own message is
 * « Failed query: … params: … » — the parameters are the device and visit
 * ids, i.e. the cookie values — so only the driver's cause and the SQLSTATE
 * are kept.
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  // Kept when well-formed, minted otherwise. Lower-cased so a hand-edited
  // cookie cannot split one browser into two devices.
  const cookieDevice = req.cookies.get(DEVICE_COOKIE)?.value;
  const cookieVisit = req.cookies.get(VISIT_COOKIE)?.value;
  const deviceId = isDeviceId(cookieDevice) ? cookieDevice.toLowerCase() : randomUUID();
  const visitId = isVisitId(cookieVisit) ? cookieVisit : newVisitId();
  ...
  after(async () => {
    try {
      ...
      console.error(`[api/visit] insert failed: ${failureText(err)}`);
    }
  });
  return response;
}
